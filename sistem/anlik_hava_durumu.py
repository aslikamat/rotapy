...
def main():
    parser = argparse.ArgumentParser(
        description="Open-Meteo Beşiktaş Anlık Hava Verisi"
    )
    parser.add_argument("--days", type=int, default=7)
    parser.add_argument("--out",  default="anlik_hava_durumu.parquet")
    args = parser.parse_args()

    df = fetch_anlik(args.days)
    save(df, args.out)

    if not df.empty:
        print(f"\n  Önümüzdeki {args.days} gün:")
