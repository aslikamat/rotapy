...
Kullanım:
    ...
    python lstm_egitim.py --epochs 20 --batch 64      # parametreler
    python lstm_egitim.py --test                      # sadece test et

Çıktı:
    besiktas_lstm_model/     → eğitilmiş model klasörü
    egitim_grafigi.png       → loss grafiği
    model_sonuclari.txt      → MAE, RMSE, doğruluk raporu
"""
...
# LSTM'e girecek feature sütunları
FEATURE_COLS = [
    # Zaman
    ...
    # Etkinlik
    "is_event", "event_attendance", "hours_to_event",
    # Hava
...
SEQUENCE_LEN = 24   # 24 saat geriye bak
...
def load_and_prepare(path: str) -> tuple:
    """
    Parquet dosyasını yükler, feature ve hedef sütunları hazırlar.
    Lokasyon bazında gruplayarak sequence oluşturur.
    """
...
def create_sequences(df: pd.DataFrame, scaler_X, scaler_y,
                     fit_scalers: bool = True) -> tuple:
    """
    LSTM için (X, y) sequence çiftleri oluşturur.
    Her X: son SEQUENCE_LEN saatin feature'ları
    Her y: bir sonraki saatin density değeri
    """
    print(f"\n[2/5] Sequence oluşturuluyor (pencere: {SEQUENCE_LEN} saat)...")

    X_all, y_all = [], []

    # Lokasyon bazında sequence oluştur (farklı lokasyonlar karışmasın)
    locations = df["location"].unique() if "location" in df.columns else ["all"]
...
def build_model(input_shape: tuple):
    """
    İki katmanlı LSTM + Dropout + Dense çıkış katmanı.
    ...
    model = Sequential([
        ...
        LSTM(
            units=64,
            return_sequences=True,   # ikinci LSTM için sequence döndür
        ...
        # Çıkış katmanı — tek değer tahmin ediyor (yoğunluk)
...
def evaluate(model, X_test, y_test, scaler_y, history) -> dict:
...
def save_results(model, scaler_X, scaler_y, results: dict,
    ...
    # Scaler'lar (inference için gerekli)
    ...
    results["sequence_len"]  = SEQUENCE_LEN

    # numpy/bool_ tiplerini JSON serializable yap
    ...
    try:
        ...
        axes[1].set_ylabel("Araç yoğunluğu")
        axes[1].legend()
        axes[1].grid(True)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Sadece 3 epoch ile hızlı test"
    )
    ...
    # TensorFlow import (geç import — kurulu değilse net hata verir)
    try:
    ...
    # 7. Değerlendir
    results, y_pred, y_true = evaluate(
        model, X_test, y_test, scaler_y, history
    )
    ...
    print(f"\n[TAMAMLANDI]")
    print(f"  Sonraki adım: bu modeli sistem/ klasörüne kopyalayın")
